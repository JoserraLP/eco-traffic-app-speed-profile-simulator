...
meta=max(pk_inicio);                        % Longitud total del recorrido
...
% Se calcula la cinemática del vehículo en cada paso de simulación
     ...
     while (espacio(paso-1) < pk_final(i))
        ...
        d_frenada(paso)=(((v_final(i))^2-velocidad(paso-1)^2)/(2*ax_brake)); 
        ...
        %   Si debe empezar la frenada, el estado=-1, con lo que a=ax_brake
            ...
            if velocidad(paso-1)>=v_final(i)  % Condicion de velocidad mayor que la final. 
                ...
                if espacio(paso-1)<(pk_final(i)-d_frenada(paso)) % No tiene que frenar, la posición
                    ...
                    if velocidad(paso-1) < (v_limite_km_h(i)/3.6)         % Si no se llega a la velocidad límite se puede acelerar
                         ...
                         % Se suaviza la aceleración, en caso de estar
                         % cerca de la velocidad límite
                         if abs(velocidad(paso-1)-(v_limite_km_h(i)/3.6))< 1
                               factor=0.25;
                         else
                             if abs(velocidad(paso-1)-(v_limite_km_h(i)/3.6))< 3
                                factor=0.5;
                             else
                                factor=1;
                             end
                         ...
                         if velocidad(paso-1) > (v_limite_km_h(i)/3.6) % En este caso tiene que frenar hasta la velocidad máxima del tramo
                           if abs(velocidad(paso-1)-(v_limite_km_h(i)/3.6))< 1
                               factor=0.25;
                           else
                             if abs(velocidad(paso-1)-(v_limite_km_h(i)/3.6))< 3
                  ...
                  % Se suaviza la aceleración, en caso de estar
                  % cerca de la velocidad límite
                         if abs(velocidad(paso-1)-(v_final(i)))< 1
                               factor=0.25;
                         else
                             if abs(velocidad(paso-1)-(v_final(i)))< 3
                                factor=0.5;
                             else
                                factor=1;
                             end
            ...
            % Se calcula una velocidad en función de la aceleración
            % impuesta en los pasos anteriores
            velocidad_calc=velocidad(paso-1)+a*delta_t;
            
            % Se suaviza la velocidad para evitar oscilaciones. 
            

                if abs(velocidad_calc-v_limite_km_h(i)/3.6)<0.1
                     velocidad(paso)=v_limite_km_h(i)/3.6;
                else
                     velocidad(paso)=velocidad_calc;
                end

            
            % Se calcula la aceleración en función de la velocidad
            % suavizada
            aceleracion_calc(paso)=(velocidad(paso)-velocidad(paso-1))/delta_t;
    ...
    if estado(j)==1
        Fuerza_traccion(j)=A+B*velocidad(j)+C*((velocidad(j))^2)+m_t*9.81*0.01*rampa_t(j)+m_t*aceleracion(j);  
            ...
            if estado(j)==0
                ...
                % Se simula el caso en el que las resistencias pasivas al 
                % avanve más la resistencia gravitatoria sean mayores que la
